...
rwt.qx.Class.define( "rwt.widgets.base.GridHeader", {
  ...
  construct : function( argsMap ) {
    ...
    //ADD
    this._fixedRight = argsMap.fixedRight;
    this._scrollWidth = 0;
    this._scrollLeft = 0;
  ...
  members : {
    ...
    setScrollLeft : function( value ) {
      ...
      if( (this._fixedColumns || this._fixedRight )&& !rwt.widgets.base.Widget._inFlushGlobalQueues ) {
        for( var i = 0; i < this._children.length; i++ ) {
          ...
          }else if(column && column.isFixedRight()){
              this._renderLabelLeft( this._children[ i ], column );
          }
        }
    ...
    _renderLabel : function( label, column ) {
      if(column.isFixedRight()){
    	  label.updateLeftBorder();
    	  var newLeft = this._computedWidthValue - column._width;
          column.setLeft(newLeft);
      }
    ...
    _renderFooterLabel : function( label, column ) {
      ...
      //ADD--||column.isFixedRight()--
     var onTop = column.isFixed() ||column.isFixedRight() || column.getFooterSpan() > 1;
    ...
    _renderHeaderLabel : function( label, column ) {
      ...
      //ADD---
      if(column.isMarkupEnabled()){
    	  label.setHtml( column.getText() );
      }else{
...
//      label.setImage( column.getImage() );
      ...
      //ADD--||column.isFixedRight()--
      label.setZIndex( (column.isFixed()||column.isFixedRight()) ? 1e7 : 1 );
    },
    ...
    _renderLabelLeft : function( label, column ) {
      var offset = (column.isFixed()||column.isFixedRight()) ? this._adjustScrollLeft( this._scrollLeft ) : 0;
