...
public class Grid extends Composite {
	...
	private boolean hideIndentionImage;
	...
	public void clearAll(boolean allChildren) {
		...
		if (itemsCount > 0) {
			// [if] Note: The parameter allChildren has no effect as all items (not only
			// rootItems)
			// are cleared
	...
	// Add---021519-
	public void setHideIndentionImage(boolean hide) {
		checkWidget();
		if (hideIndentionImage != hide) {
			hideIndentionImage = hide;
			scheduleRedraw();
		}
	}

	public boolean getHideIndentionImage() {
		checkWidget();
		return hideIndentionImage;
	}

	public void setOverlayWidth(int overlayWidth) {
		checkWidget();
		if (this.overlayWidth != overlayWidth) {
	...
	private int getCheckBoxOffset(int index) {
		...
		if (isColumnCentered(index) && !isTreeColumn(index) && !hasColumnImages(index) && !hasColumnTexts(index)) {
	...
	private static int checkStyle(int style) {
		int mask = SWT.BORDER | SWT.LEFT_TO_RIGHT
		// | SWT.RIGHT_TO_LEFT
				| SWT.H_SCROLL | SWT.V_SCROLL | SWT.SINGLE | SWT.MULTI | SWT.NO_FOCUS | SWT.CHECK | SWT.VIRTUAL;
		...
		if (result != null) {
			...
			if (availableHeight <= 0) {
				...
				while (consumedHeight + 1 <= availableHeight && !abort) {
					...
					if (nextItem == null || forceEndCompletelyInside
	...
	private final class GridAdapter
		...
		public boolean isFixedColumn(GridColumn column) {
			return Grid.this.isFixedColumn(Grid.this.indexOf(column));
		}

		@Override
		public int getRightFixedColumnWidth() {
			return Grid.this.getRightFixedColumnWidth();
		}
		// ADD----
	...
	public int getRightFixedColumnWidth() {
		...
		for (int i = 0; i < cols.length; i++) {
			Object fr = cols[i].getData("fixedRight");
			if (fr != null) {
				return cols[i].getWidth();
			}
		}
		return 0;
	}

	public boolean isFixedColumn(int index) {
