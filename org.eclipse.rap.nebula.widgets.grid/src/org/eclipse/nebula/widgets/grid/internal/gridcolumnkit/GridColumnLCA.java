...
public class GridColumnLCA extends WidgetLCA<GridColumn> {
	...
	private static final String PROP_FIXED_RIGHT = "fixedRight";
	// ADD----
	...
	public void renderInitialization(GridColumn column) throws IOException {
		...
		// ADD------------
		remoteObject.set(PROP_MARKUP_ENABLED, MarkupUtil.isMarkupEnabledFor(column));
		remoteObject.set(PROP_FIXED_RIGHT, Boolean.TRUE.equals(column.getData(PROP_FIXED_RIGHT)));
		// ADD------------
	...
	private static boolean isFixed(GridColumn column) {
		IGridAdapter adapter = column.getParent().getAdapter(IGridAdapter.class);
