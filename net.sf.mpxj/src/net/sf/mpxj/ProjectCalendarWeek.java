...
public class ProjectCalendarWeek implements Comparable<ProjectCalendarWeek> {
	...
	/**
	 * Retrieves the data range for which this week is valid. Returns null if this
	 * is the default week.
	 *
	...
	/**
	 * Sets the date range for this week. Set this to null to indicate the default
	 * week.
	 *
	 * @param range
	 *            date range, or null
	...
	/**
	 * If this week is derived from a another week, this method will return the
	 * parent week.
	 *
	 * @return parent week
	 */
	...
	/**
	 * Adds a set of hours to this calendar without assigning them to a particular
	 * day.
	 *
	...
	/**
	 * This method retrieves the calendar hours for the specified day. Note that
	 * this method only returns the hours specification for the current calendar.If
	 * this is a derived calendar, it does not refer to the base calendar.
	 *
	 * @param day
	 *            Day instance
	...
	/**
	 * Retrieve an array representing all of the calendar hours defined by this
	 * calendar.
	 *
	...
	/**
	 * This method retrieves the calendar hours for the specified day. Note that if
	 * this is a derived calendar, then this method will refer to the base calendar
	 * where no hours are specified in the derived calendar.
	 *
	 * @param day
	 *            Day instance
	 * @return calendar hours
	...
	public ProjectCalendarHours getHours(Day day) {
		...
		if (result == null) {
			...
			//
			if (m_parent == null) {
				// Only add default hours for the day that is 'missing' to avoid overwriting
				// real calendar hours
	...
	/**
	 * This is a convenience method used to add a default set of calendar hours to a
	 * calendar.
	 */
	...
	public void add7x24CalendarHours() {
		for (int i = 1; i <= 7; i++) {
			add7x24CalendarHours(Day.getInstance(i));
		}
	}
	 ...
	 *
	 * @param day
	 *            Day for which to add default hours for
	 */
	...
	public void add7x24CalendarHours(Day day) {
		ProjectCalendarHours hours = addCalendarHours(day);
		hours.addRange(new DateRange(DateHelper.getTime(0, 0), DateHelper.getTime(24, 0)));
	}

	/**
	 * Used to add working hours to the calendar. Note that the MPX file definition
	 * allows a maximum of 7 calendar hours records to be added to a single
	 * calendar.
	 *
	 * @param day
	 *            day number
	...
	/**
	 * This method allows the retrieval of the actual working day flag, which can
	 * take the values DEFAULT, WORKING, or NONWORKING. This differs from the
	 * isWorkingDay method as it retrieves the actual flag value. The isWorkingDay
	 * method will always refer back to the base calendar to get a boolean value if
	 * the underlying flag value is DEFAULT. If isWorkingDay were the only method
	 * available to access this flag, it would not be possible to determine that a
	 * resource calendar had one or more flags set to DEFAULT.
	 *
	 * @param day
	 *            required day
	...
	/**
	 * This is a convenience method provided to allow a day to be set as working or
	 * non-working, by using the day number to identify the required day.
	 *
	 * @param day
	 *            required day
	 * @param working
	 ...
	 */
	public static final DateRange DEFAULT_WORKING_MORNING = new DateRange(DateHelper.getTime(8, 0),
			DateHelper.getTime(12, 0));
	public static final DateRange DEFAULT_WORKING_AFTERNOON = new DateRange(DateHelper.getTime(13, 0),
			DateHelper.getTime(17, 0));
}
