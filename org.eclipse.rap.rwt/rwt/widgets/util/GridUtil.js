...
rwt.widgets.util.GridUtil = {
  ...
  //ADD
  setFixedRight  : function( tree, value ) {
    var container = tree.getRowContainer();
    if( container.setFixedRight ) {
      container.setFixedRight( value );
      tree.update();
    }
   },
  //ADD
  ...
  createTreeRowContainer : function( argsmap ) {
    var result;
    if( argsmap.splitContainer || argsmap.fixedRight) {
  ...
  getColumnByPageX : function( grid, pageX ) {
    ...
    if( result === -1 && splitContainer ) {
      ...
      //ADD
      if(result === -1){
          container = grid.getRowContainer().getSubContainer( 2 );
          result = this._getColumnByPageX( container, pageX );
      }
      //ADD
