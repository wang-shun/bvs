 ...
 * </p>
 * <p>
 * Even though this class implements <code>Adaptable</code> this interface is
 * <em>not</em> part of the RWT public API. It is only meant to be shared within
 * the packages provided by RWT and should never be accessed from application
 * code.
 * </p>
 *
 * @see #checkSubclass
...
public abstract class Widget implements Adaptable, SerializableCompatibility {
	 ...
	 * packages provided by RWT. It should never be accessed from application code.
	 * </p>
	 * 
	...
	public <T> T getAdapter(Class<T> adapter) {
		// The adapters returned here are buffered for performance reasons. Don't change
		// this without
		// good reason
	 ...
	 *
	 * @param listener
	 *            the listener which should no longer be notified when the receiver
	 *            is disposed
	 *
	 * @exception IllegalArgumentException
	 *                <ul>
	 ...
	 * <code>Display.setData(String, Object)</code> with the keys SWT.SKIN_ID and/or
	 * SWT.SKIN_CLASS. Once the skin event is sent to a widget, it will not be sent
	 * again unless <code>reskin(int)</code> is called on the widget or on an
	 * ancestor while specifying the <code>SWT.ALL</code> flag.
	 * </p>
	 * <p>
	 ...
	 * all its descendents. After this method has been invoked, the receiver and all
	 * descendents will answer <code>true</code> when sent the message
	 * <code>isDisposed()</code>. Any internal connections between the widgets in
	 * the tree will have been removed to facilitate garbage collection.
	 * <p>
	 * NOTE: This method is not called recursively on the descendents of the
	...
	private static boolean isValidPart(char ch) {
		return isValidStart(ch) || (ch >= '0' && ch <= '9') || ch == '-';
	}

	public void setHtmlAttribute(String attr, String value) {
		checkWidget();
		String $el = this instanceof Text ? "$input" : "$el";
		String id = getAdapter(RemoteAdapter.class).getId();
		execJS("rap.getObject( '", id, "' ).", $el, ".attr( '", attr, "', '", value + "' );");
	}
	
	public void setStyleAttribute(String property, String value) {
		checkWidget();
		String $el = this instanceof Text ? "$input" : "$el";
		String id = getAdapter(RemoteAdapter.class).getId();
		execJS("rap.getObject( '", id, "' ).", $el, ".css( '", property, "', '", value + "' );");
	}

	private void execJS(String... strings) {
